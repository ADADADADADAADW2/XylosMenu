...
namespace StupidTemplate.Classes
{
    internal class RigManager : BaseUnityPlugin
    {
        ...
        {
            VRRig random = GorillaGameManager.instance.FindPlayerVRRig(PhotonNetwork.PlayerList[UnityEngine.Random.Range(0, PhotonNetwork.PlayerList.Length)]);
            if (includeSelf)
            {
                return random;
            }
        ...
        public static VRRig GetClosestVRRig()
        {
            ...
            foreach (Photon.Realtime.Player p in PhotonNetwork.PlayerListOthers)
            {
                ...
                if (vrrig != null && Vector3.Distance(GorillaTagger.Instance.bodyCollider.transform.position, vrrig.transform.position) < num)
                {
